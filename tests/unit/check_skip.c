...
START_TEST(yps_noop)
{
    ...
    ck_assert(yaml_parser_skip(&parser, yps_noop_event_types[_i]));
...
START_TEST(yps_stream_empty)
{
    ...
    ck_assert(yaml_parser_skip(&parser, YAML_STREAM_START_EVENT));
...
START_TEST(yps_one_document)
{
    ...
    ck_assert(yaml_parser_skip(&parser, YAML_DOCUMENT_START_EVENT));
...
START_TEST(yps_mapping_basic)
{
    ...
    ck_assert(yaml_parser_skip(&parser, YAML_MAPPING_START_EVENT));
...
START_TEST(yps_nested_mappings_inner)
{
    ...
    /* Check the previous YAML_MAPPING_END_EVENT refers to the outer mapping */
    ck_assert(yaml_parser_parse(&parser, &event));
    ck_assert_int_eq(event.type, YAML_DOCUMENT_END_EVENT);
    yaml_event_delete(&event);
}
...
START_TEST(yps_sequence_basic)
{
    ...
    ck_assert(yaml_parser_skip(&parser, YAML_SEQUENCE_START_EVENT));
...
START_TEST(yps_nested_sequences_inner)
{
    ...
    /* Check the previous YAML_SEQUENCE_END_EVENT refers to the outer mapping */
    ck_assert(yaml_parser_parse(&parser, &event));
    ck_assert_int_eq(event.type, YAML_DOCUMENT_END_EVENT);
    yaml_event_delete(&event);
}
